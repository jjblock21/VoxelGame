...
{
    // Simplex Noise for C#
    // Copyright © Benjamin Ward 2019. All rights reserved.
    // Original repository: https://github.com/WardBenjamin/SimplexNoise
    // Simplex Noise implementation offering 1D, 2D, and 3D forms.
    // Based on work by Heikki Törmälä (2012) and Stefan Gustavson (2006).
    /// <summary>
    /// Implementation of the Perlin simplex noise, an improved Perlin noise algorithm.<br/>
    /// Based loosely on SimplexNoise1234 by Stefan Gustavson: http://staffwww.itn.liu.se/~stegu/aqsis/aqsis-newnoise/<br/>
    /// </summary>
    public class SimplexNoise
    {
        ...
        /// <param name="permTable">Initial value for the permutation table value.</param>
        public SimplexNoise(byte[] permTable)
        {
            _perm = permTable;
        }

        /// <summary>
        /// Generate the permutation table for a given seed.
        /// </summary>
        public static byte[] GenPermTable(int seed)
        {
        ...
        /// <summary>
        /// Change the permutation table used for generating the noise.
        /// </summary>
        public void SetPermTable(byte[] perm)
        {
            if (perm.Length != 512)
                throw new ArgumentException("Permutation table should contain 512 values.");
            _perm = perm;
        }

        /// <summary>
        /// Samples a Simplex Noise value between 0 and 1.
        /// </summary>
        public float Sample1D(int x) =>
            Generate(x * Scale) * 0.5f + 0.5f;

        /// <summary>
        /// Samples a Simplex Noise value between 0 and 1.
        /// </summary>
        public float Sample2D(int x, int y) =>
            Generate(x * Scale, y * Scale) * 0.5f + 0.5f;

        /// <summary>
        /// Samples a Simplex Noise value between 0 and 1.
        /// </summary>
        public float Sample3D(int x, int y, int z) =>
            Generate(x * Scale, y * Scale, z * Scale) * 0.5f + 0.5f;
